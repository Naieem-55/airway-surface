...
public:
    ...
    // common property
    float shininess;
    bool wantShallow = false;


    // constructors
    Cylinder(bool wantShallow = false)
    {
        this->wantShallow = wantShallow;

        if (this->wantShallow) {
            this->setUpCylinderVertexDataAndConfigureVertexAttributeWithInnerRadiusHollowPart();
        }
        else {
    ...
    Cylinder(glm::vec3 amb, glm::vec3 diff, glm::vec3 spec, float shiny, bool wantShallow=false)
    {
        this->ambient = amb;
        this->diffuse = diff;
        this->specular = spec;
    ...
    Cylinder(unsigned int dMap, unsigned int sMap, float shiny, float textureXmin, float textureYmin, float textureXmax, float textureYmax, bool wantShallow = false)
    {
    ...
    void drawCylinderWithTexture(Shader& lightingShaderWithTexture, glm::mat4 model = glm::mat4(1.0f))
    {
        ...
        // Each segment needs 6 indices for the sides (2 triangles)
        // plus 6 more for the top and bottom caps (2 triangles)
        int totalIndices = (SEGMENTS * 6) + (SEGMENTS * 6);
...
private:
    ...
    void setUpCylinderVertexDataAndConfigureVertexAttributeWithInnerRadiusHollowPart()
    {
        const float OUTER_RADIUS = 2.0f;
        const float INNER_RADIUS = 1.8f;  // Controls how hollow it is
        ...
        for (int i = 0; i <= SEGMENTS; ++i) {
            float angle = (80.0f * 3.1416f * i) / SEGMENTS;
            float cosA = cos(angle);
            float sinA = sin(angle);

            // Outer circle top vertex
            float x_outer = OUTER_RADIUS * cosA;
            float z_outer = OUTER_RADIUS * sinA;
            cylinder_vertices.push_back(x_outer);     // x
            cylinder_vertices.push_back(HEIGHT);      // y
            cylinder_vertices.push_back(z_outer);     // z
            cylinder_vertices.push_back(x_outer);     // normal x
            cylinder_vertices.push_back(HEIGHT);      // normal y
            cylinder_vertices.push_back(z_outer);     // normal z
            cylinder_vertices.push_back(float(i) / SEGMENTS);  // texture u
            cylinder_vertices.push_back(TYmax);              // texture v

            // Outer circle bottom vertex
            cylinder_vertices.push_back(x_outer);     // x
            cylinder_vertices.push_back(-HEIGHT);     // y
            cylinder_vertices.push_back(z_outer);     // z
            cylinder_vertices.push_back(x_outer);     // normal x
            ...
            cylinder_vertices.push_back(z_outer);     // normal z
            cylinder_vertices.push_back(float(i) / SEGMENTS);  // texture u
            cylinder_vertices.push_back(TYmin);              // texture v

            // Inner circle top vertex
            float x_inner = INNER_RADIUS * cosA;
            float z_inner = INNER_RADIUS * sinA;
            cylinder_vertices.push_back(x_inner);     // x
            cylinder_vertices.push_back(HEIGHT);      // y
            cylinder_vertices.push_back(z_inner);     // z
            cylinder_vertices.push_back(-x_inner);    // normal x (pointing inward)
            cylinder_vertices.push_back(HEIGHT);      // normal y
            cylinder_vertices.push_back(-z_inner);    // normal z
            cylinder_vertices.push_back(float(i) / SEGMENTS);  // texture u
            cylinder_vertices.push_back(TYmax);              // texture v

            // Inner circle bottom vertex
            cylinder_vertices.push_back(x_inner);     // x
            cylinder_vertices.push_back(-HEIGHT);     // y
            cylinder_vertices.push_back(z_inner);     // z
            cylinder_vertices.push_back(-x_inner);    // normal x (pointing inward)
        ...
        for (int i = 0; i < SEGMENTS; ++i) {
            int current = i * 4;       // Each vertex set now has 4 vertices
            int next = (i + 1) * 4;

            // Outer wall
            cylinder_indices.push_back(current);      // Top outer
            cylinder_indices.push_back(current + 1);  // Bottom outer
            cylinder_indices.push_back(next);         // Top outer next

            cylinder_indices.push_back(next);         // Top outer next
            cylinder_indices.push_back(current + 1);  // Bottom outer
            cylinder_indices.push_back(next + 1);     // Bottom outer next

            // Inner wall
            cylinder_indices.push_back(current + 2);  // Top inner
            cylinder_indices.push_back(next + 2);     // Top inner next
            cylinder_indices.push_back(current + 3);  // Bottom inner

            cylinder_indices.push_back(next + 2);     // Top inner next
            cylinder_indices.push_back(next + 3);     // Bottom inner next
            cylinder_indices.push_back(current + 3);  // Bottom inner

            // Top rim
            cylinder_indices.push_back(current);      // Outer top
            cylinder_indices.push_back(next);         // Outer top next
            cylinder_indices.push_back(current + 2);  // Inner top

            cylinder_indices.push_back(next);         // Outer top next
            cylinder_indices.push_back(next + 2);     // Inner top next
            cylinder_indices.push_back(current + 2);  // Inner top

            // Bottom rim
            cylinder_indices.push_back(current + 1);  // Outer bottom
            cylinder_indices.push_back(current + 3);  // Inner bottom
            cylinder_indices.push_back(next + 1);     // Outer bottom next

            cylinder_indices.push_back(next + 1);     // Outer bottom next
            cylinder_indices.push_back(current + 3);  // Inner bottom
            cylinder_indices.push_back(next + 3);     // Inner bottom next
        }
        ...
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, cylinderEBO);
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, cylinder_indices.size() * sizeof(unsigned int), &cylinder_indices[0], GL_STATIC_DRAW);
